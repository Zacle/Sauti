...
Managed tool webhook: https://sauti.uk/webhooks/telnyx/tools/{toolName}
```

Telnyx owns live media, speech recognition, synthesis, and interruption
handling. Sauti does not expose a telephony media WebSocket. Managed tool
requests must include the server-only `TELNYX_TOOL_WEBHOOK_SECRET`.
...
### 2026-07-24 - Make managed booking lookup and call completion deterministic

- Diagnosed the three provider reports supplied in this turn:
  - Retell `36a6de45-530b-485a-b18a-4b536d2583ee` disconnected at roughly 132 seconds after repeated provider speaking-state events. The Retell browser SDK did not expose a disconnect reason.
  - ElevenLabs `5bc17524-7e6f-4933-98e6-f62431b3c211` emitted `Server error: Unknown error` about 176 ms after its greeting transcript, before caller speech or any Sauti tool execution.
  - Telnyx `d5a987e7-126a-4514-b471-3f092759f4ab` invoked `update_conversation_state` six times but never invoked `lookup_booking`, a mutation tool, or `end_call`; the booking-not-found statement therefore had no database result behind it.
- Removed the provider model as a point of failure for exact server-authorized transitions:
  - once a newly supplied booking number and phone make an identity complete, `ConversationStateTool` deterministically authorizes `lookup_booking`;
  - `ManagedVoiceToolService` now follows bounded exact `nextTool` + `nextToolArguments` transitions generated by Sauti, including an empty argument object, with loop and depth protection;
  - terminal actions remain outside the generic server chain because Retell and ElevenLabs use native provider end-call actions.
- Added language-neutral `call_disposition=continue|end` to the required semantic state contract. The model decides meaning in the caller's language; Sauti deterministically maps `end` to `end_call` and passes the model-authored respectful farewell. No language-specific farewell phrase matching or hard-coded translated speech was added.
- Fixed managed Telnyx termination:
  - terminal-result recognition now accepts both the native nested result and Sauti's managed top-level/data envelope;
  - the Telnyx client follows an exact authorized terminal transition rather than waiting for another model choice;
  - the runtime arms completion after the terminal result, waits for farewell speech to start and drain, and uses a 12-second safety fallback rather than disconnecting before speech or hanging indefinitely.
- Improved provider diagnostics:
  - Retell test-call diagnostics now record the non-secret provider call ID, configured maximum duration, and configured silence end threshold;
  - ElevenLabs runtime errors now include the non-secret conversation ID when the SDK has assigned one, allowing the provider-side server error to be traced without exposing the API key or retrying a greeting.
- Bumped managed-provider configuration versions so existing generated agents receive the new semantic schema and execution contract:
  - Retell: `5`;
  - ElevenLabs: `9`;
  - Telnyx: `9`.
- Files touched:
  - `backend/src/main/java/com/sauti/call/{ElevenLabsManagedVoiceAgentProvisioner,ManagedVoiceToolService,RetellBrowserVoiceRuntimeService,RetellManagedVoiceAgentProvisioner,TelnyxManagedVoiceAgentProvisioner}.java`
  - `backend/src/main/java/com/sauti/tool/ConversationStateTool.java`
  - `backend/src/test/java/com/sauti/call/{ManagedBrowserVoiceRuntimeServicesTest,ManagedVoiceToolServiceTest}.java`
  - `backend/src/test/java/com/sauti/tool/ConversationStateToolTest.java`
  - `dashboard/features/agents/AgentCreator/TestCallPanel.tsx`
  - `dashboard/features/voice-runtime/{elevenLabsRuntime,openaiRealtime.test,realtimeProtocol,telnyxClientTool,telnyxClientTool.test,telnyxRuntime}.ts`
  - `docs/agent-handoff.md`
- Verification:
  - focused conversation-state, managed-tool, provider-provisioning, and browser-runtime backend tests passed.
  - `.\gradlew.bat :backend:test --rerun-tasks` passed; Gradle reported `BUILD SUCCESSFUL` in 1 minute 24 seconds.
  - `npm.cmd run test:voice` passed all 51 tests.
  - `npm.cmd run typecheck` passed.
  - `npm.cmd run lint` passed with zero warnings.
  - `npm.cmd run build` passed; Next.js completed the optimized production build.
  - `git diff --check` passed (line-ending notices only).
...
- Required live verification after reviewed deployment:
  - Telnyx: supply a valid booking number and phone, verify diagnostics show `lookup_booking`, reschedule it, then say goodbye. Diagnostics should show the chained `end_call`, farewell speech, `runtime_ended`, and no manual End action.
  - ElevenLabs: if the greeting server error recurs, provide the newly logged ElevenLabs conversation ID to provider support; do not add a blind reconnect that duplicates greetings.
  - Retell: compare `runtime_ended.elapsedMs` with the new `maxCallDurationSeconds`. A close match confirms the configured duration limit; an earlier end with no caller/native end action should be escalated with the logged Retell provider call ID.
### 2026-07-24 - Consolidate all live voice paths on Telnyx

- Replaced the multi-provider experiment architecture with one Telnyx runtime for browser test calls, public Web Voice, inbound telephone calls, outbound reminders/callbacks, transfers, recordings, voice previews, and native call termination.
- Removed the Retell, ElevenLabs, Vapi, Cartesia, OpenAI Realtime, hybrid audio, Twilio media, SignalWire, and custom telephony-media WebSocket implementations and their provider-specific dashboard adapters/dependencies. General OpenAI/Gemini LLM support and prerecorded transcription used by non-live channels remain independent of the live voice runtime.
- Telnyx assistant provisioning is now the only managed-agent path:
  - assistants receive the selected native Telnyx voice and multilingual `deepgram/nova-3` transcription configuration;
  - Sauti business tools are authenticated server-side Telnyx webhook tools;
  - synchronous webhook tools explicitly allow 30 seconds instead of Telnyx's
    approximately 5.25-second default, preventing guarded calendar and
    integration work from being cut off at the old failure boundary;
  - slow tools instruct the model to give one brief contextual progress acknowledgement, then continue automatically from the returned factual result;
  - `end_call` records the terminal outcome and is followed by Telnyx's native `hang_up` tool.
- Added a server-only Telnyx voice catalog/preview client. The Agent Studio voice picker now lists, filters, previews, and selects Telnyx voices and languages without exposing the API key.
- Added a safe default voice (`TELNYX_DEFAULT_VOICE_ID`, default `Telnyx.NaturalHD.astra`) for legacy agents whose stored voice ID belongs to a removed provider.
- Migrated phone lifecycle features:
  - outbound scheduled calls use Telnyx Call Control and start the managed assistant after `call.answered`;
  - live transfers use Telnyx's transfer action;
  - remote recording downloads use Telnyx bearer authentication;
  - the seeded fulfillment type is now provider-neutral `call_transfer`.
- Added Flyway migrations:
  - `V37__expand_agent_voice_id_for_telnyx.sql` expands the stored voice ID for compound Telnyx identifiers;
  - `V38__rename_transfer_fulfillment_for_telnyx.sql` migrates `twilio_transfer` tool rows to `call_transfer`.
- Production now requires `sauti.telephony.provider=telnyx`; obsolete streaming-provider and media-WebSocket configuration was removed from application, deployment, and example environment files.
- The deploy workflow securely synchronizes `TELNYX_API_KEY`,
  `TELNYX_PUBLIC_KEY`, `TELNYX_CONNECTION_ID`, and
  `TELNYX_TOOL_WEBHOOK_SECRET` from GitHub Actions secrets into the server
  environment before the normal reviewed deployment.
- Required provider environment:
  - `TELNYX_API_KEY`
  - `TELNYX_PUBLIC_KEY`
  - `TELNYX_CONNECTION_ID`
  - `TELNYX_TOOL_WEBHOOK_SECRET`
  - `PUBLIC_BASE_URL`
  - optional `TELNYX_DEFAULT_VOICE_ID`, `TELNYX_AI_ENVIRONMENT`, `TELNYX_AI_REGION`, `TELNYX_REQUIREMENT_GROUP_ID`, `TELNYX_OUTBOUND_ENABLED`, `TELNYX_OUTBOUND_FROM_NUMBER`, and `TELNYX_OUTBOUND_POLL_DELAY_MS`.
- Major files touched:
  - Telnyx voice/catalog/provisioning/call control under `backend/src/main/java/com/sauti/{agent,api,call,telnyx,voice}`;
  - outbound, transfer, recording, and tool routing services;
  - Agent Studio Test Call, Voice Picker, public Web Voice, and the Telnyx dashboard runtime;
  - application/deployment environment configuration, migrations, provider dependencies, tests, and marketing copy.
- Verification:
  - `.\gradlew.bat :backend:test` - passed, 248 tests.
  - `npm.cmd run typecheck` - passed.
  - `npm.cmd run lint` - passed with zero warnings.
  - `npm.cmd run test:voice` - passed, 6 tests.
  - `npm.cmd run build` - passed; Next.js produced the optimized production build.
  - `git diff --check` - passed (line-ending notices only).
- Deployment status: not deployed. All changes remain uncommitted for maintainer review and the normal GitHub Actions CI/CD flow.
- Live verification still required after reviewed deployment:
  - confirm Telnyx accepts the generated assistant payload, including inline webhook and native hang-up tool schemas;
