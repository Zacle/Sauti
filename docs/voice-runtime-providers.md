...
Agent Studio and public web hybrid calls use the official `@cartesia/cartesia-js` context/WebSocket client with a server-minted, short-lived TTS-only access token. Cartesia audio therefore travels directly to the browser instead of crossing the Sauti backend WebSocket. The maintained `@speechmatics/web-pcm-player` package owns Web Audio chunk scheduling at 48 kHz; Sauti does not resample, adaptively accumulate, or schedule individual browser chunks on this path. The old backend-relayed PCM implementation remains only as a compatibility path for non-hybrid transports, while the server-side Cartesia client remains authoritative for telephony and other trusted server channels.

The account API key never reaches the browser. Session-start responses contain a call-scoped token with only the Cartesia `tts` grant, clamped to a maximum one-hour lifetime, plus the selected provider voice and model IDs. `CARTESIA_ACCESS_TOKEN_URL` defaults to `https://api.cartesia.ai/access-token` and normally does not need to be changed.

Assistant transcripts are persisted as soon as OpenAI completes them, but hybrid captions are presented only when the first Cartesia audio chunk enters audible playback. Cached greetings follow the same playback-start rule. Interruption cancels the active Cartesia context, clears queued speech, and resolves any pending drain wait so the next valid response cannot become stuck behind interrupted audio.
...
Potentially remote tools now receive a model-generated progress update after 1.5 seconds and another short update every 8 seconds while the operation remains pending. The model chooses natural wording in the active caller language; Sauti supplies only the factual lifecycle state and prohibits invented success or failure. Internal conversation-state updates, static business-hours reads, and call ending stay on the immediate path. Each progress update uses `conversation: none`: it is an out-of-band lifecycle notification, not a conversational turn, and it never waits for caller acknowledgement.

If the caller interrupts a progress update, playback stops and the new caller turn takes priority, but the accepted tool future remains alive. Semantically identical provider retries share that active execution. When the operation completes, Sauti cancels any progress response that is still generating, adds the function result to conversation history, and proactively speaks or queues the factual outcome. No caller speech is needed to resume the call. Agent Studio displays `Still working on your request…` while the operation is pending, even though the microphone remains open for optional interruption.

Server-authorized chained tools are represented as a native Realtime `function_call` followed by its matching `function_call_output`, including fast reads such as business hours. The model therefore receives the actual database/configuration result before composing the answer instead of improvising from an invisible server-side operation. This same transport contract covers reads and authorized create, update, and delete actions.
