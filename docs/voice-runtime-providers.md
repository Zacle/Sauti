...
The hybrid browser media path coalesces Cartesia's arbitrary provider chunks into 40 ms PCM WebSocket frames before forwarding them. The AudioWorklet starts with a 280 ms jitter buffer and increases its target by 80 ms after a real underrun, up to 720 ms. It also learns from uncovered PCM arrival gaps. The learned target is retained across responses and legitimate interruption clears for the rest of the call; resetting it after each sentence made the same network repeatedly underrun. The browser no longer treats a 1.2-second delivery gap as a failed utterance. Cartesia liveness belongs to the backend's eight-second provider watchdog, with a ten-second browser fallback only if that server event is lost. This intentionally adapts only after observed jitter rather than adding the maximum delay to every call. Interruption still clears queued audio and closes the old Cartesia context immediately.

Each browser underrun reports both the low-cardinality `sauti.voice.playback.underruns` counter and the selected rebuffer target through `sauti.voice.playback.rebuffer.target`. Use those measurements to separate PCM starvation from model, tool, or speech-synthesis latency.
