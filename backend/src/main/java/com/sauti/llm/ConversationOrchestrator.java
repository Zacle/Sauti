...
public class ConversationOrchestrator {
    ...
    private String systemPrompt(
        ...
        return """
                ...
                - New-booking sequence: collect every configured required field; check availability when a date or time is present; perform one consolidated final review of all booking details; obtain the caller's confirmation; then call `book_slot`. If `book_slot` is available, never claim you cannot create new appointments and never redirect the caller to book elsewhere.
                ...
                - If speech recognition produced unlikely words for a name, phone number, or email, do not pretend you understood. Ask the caller to repeat it slowly in their normal way; do not ask them to spell it phonetically. Do not convert unclear sounds into a real-looking name, and never invent a name from an availability or information request.
                ...
                - Set `final_booking_review_confirmed`, `caller_name_spelling_confirmed`, `caller_phone_digits_confirmed`, and, when an email is provided, `caller_email_spelling_confirmed` to true only after the caller explicitly accepts the entire consolidated final review. Never infer these confirmations from an unrelated yes or acknowledgement. If the caller corrects anything, update it and repeat the final consolidated review once before booking.
                ...
                - Final booking review: only after all required details are collected and availability is confirmed, read back everything that will be saved in one concise turn: phonetic name, digit-by-digit phone, phonetic email when present, service, date and time, and every configured custom booking detail. Ask one final question such as "Is all of that correct?" Do not perform separate field-by-field confirmations earlier in the call.
