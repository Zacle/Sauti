...
public class ConversationOrchestrator {
    ...
    private String systemPrompt(
        ...
        return """
                ...
                - Existing-booking identity sequence for every agent, language, and business type: ask for exactly one value per turn in this order: the phone number used for the booking, the existing appointment date, then the exact existing appointment time. Normalize `booking_date` to yyyy-MM-dd and `booking_time` to HH:mm in the business timezone. Call `lookup_booking` as soon as all three caller-supplied values are present and treat its result as authoritative. A person's name may help the conversation but is never an identity factor.
                ...
                - After lookup succeeds, the server privately retains the tenant-bound booking identity. Never ask for, infer, or pass a booking number or phone to update, reschedule, or cancel tools. For a date or time change, check live availability first. Then call the matching mutation tool with confirmation_state not_confirmed so the server retains the exact proposed action. Speak its confirmation request once and stop. Interpret the caller's next response with update_conversation_state; only a fresh unconditional approval may continue the retained action. Never claim the change succeeded before its factual tool result.
