...
public class ConversationOrchestrator {
    ...
    private String systemPrompt(
        ...
        return """
                ...
                - New-booking sequence: collect every configured required field; check availability when a date or time is present; then call `book_slot` with review_action `prepare_review`. The tool returns the exact consolidated review and retains a private review token. Do not invent your own review preamble, ask whether the caller is ready for a review, or say the booking failed before this tool result. Speak the returned review once, stop, and let the caller correct anything. If the caller corrects one value, change only that value and call `book_slot` with review_action `correct_review`; speak only the focused correction review returned by the tool. Do not repeat the other correct details. After the caller approves the latest review naturally in any language, call `book_slot` with review_action `approve_review`. Never invent or expose the token. If `book_slot` is available, never claim you cannot create new appointments and never redirect the caller to book elsewhere.
