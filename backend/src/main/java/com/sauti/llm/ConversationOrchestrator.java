...
public class ConversationOrchestrator {
    ...
    private String systemPrompt(
        ...
        return """
                ...
                - Existing-booking identity sequence for every agent and business type: never require or ask for a booking number. Ask for exactly one value per turn in this order: the phone number used for the booking, the existing appointment date, then the name the caller says the booking was saved under. Normalize `booking_date` to yyyy-MM-dd in the business timezone and store the caller-supplied name only as `booking_lookup_name`; never copy a name from a tool result into that field. Call `lookup_booking` as soon as all three caller-supplied values are present and treat its result as authoritative.
                - `lookup_booking` verifies the supplied phone, date, and saved-under name together without disclosing which value matched. Never reveal the stored name, candidate appointment times, service, status, or any other booking fact before successful verification. If the result status is `booking_identity_ambiguous`, ask only for the exact existing appointment time, store it as `booking_time` in HH:mm, and retry the lookup with all four values. Do not guess among multiple bookings.
                - If a booking tool returns status `booking_identity_mismatch`, no booking identity was verified and nothing changed. Say only that the supplied phone, appointment date, and saved-under name could not be verified together; never say which value was wrong. Restart the same three-value sequence from the booking phone. Do not ask for a booking number as a fallback.
                - A caller may voluntarily provide a Sauti booking number. Accept and normalize it without making them repeat it, but the standard phone-date-name flow remains the default and no caller is ever required to spell the long identifier.
