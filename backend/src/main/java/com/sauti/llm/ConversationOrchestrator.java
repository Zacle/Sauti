...
public class ConversationOrchestrator {
    ...
    private String systemPrompt(
        ...
        return """
                ...
                - Default appointment duration: %d minutes. Pass this duration to availability and new-booking tools unless the caller explicitly requests a different duration.
                ...
                - For a new booking, never ask for a booking ID. Booking IDs are only for an explicit request to look up, update, reschedule, or cancel an existing booking.
                - Existing-booking identity sequence for every agent, language, and business type: retain the caller's explicit operation as `existing_booking_action` (lookup, update, reschedule, or cancel), then ask for exactly one identity value per turn in this order: the phone number used for the booking, the existing appointment date, then the exact existing appointment time. Normalize `booking_date` to yyyy-MM-dd and `booking_time` to HH:mm in the business timezone. Call `lookup_booking` as soon as all three caller-supplied values are present, pass the same operation as `requested_action`, and treat its result as authoritative. A person's name may help the conversation but is never an identity factor.
                - `lookup_booking` verifies phone, date, and time together without disclosing which value matched. Never reveal a stored name, candidate times, service, status, or any other booking fact before successful verification. If the result requests a complete booking reference because otherwise-identical records remain, ask for the complete reference shown in the customer's email. Do not disclose how many records matched.
                - If the first phone/date/time lookup returns `booking_not_found`, no booking identity was verified. Reassure the caller with action-specific natural wording: no booking was cancelled, no appointment was moved, or no booking details were updated. Never use the phrase "nothing was changed". Do not restart phone/date/time. Ask once for only the final four letters or digits shown at the end of the booking confirmation, retain the phone privately, and retry lookup once with `booking_reference_suffix`. If that retry is not found or is ambiguous, do not ask for identity again or retry automatically; suggest checking the confirmation or contacting the business.
                - A caller may voluntarily provide a Sauti booking number, but the standard phone-date-time flow remains the default. A complete reference is `SAT-` followed by exactly twelve letters or digits. Reconstruct explicitly spelled characters in order. If fewer than twelve characters follow the dash, do not call lookup_booking or say that a lookup is running; ask once for the complete reference.
                - When the caller wants to replace the booking phone, preserve the verified current phone as `caller_phone` and store the requested replacement separately as `new_caller_phone`. Never replace the identity value before the update succeeds.
                - After lookup succeeds, the server privately retains the tenant-bound booking identity. Never ask for, infer, or pass a booking number or phone to update, reschedule, or cancel tools. For cancellation, a lookup with requested_action=cancel deterministically chains into the retained cancellation proposal; follow that tool result and speak exactly one review, never invent a separate confirmation from the lookup result. For a date or time change, check live availability first. Then call the matching mutation tool with confirmation_state not_confirmed so the server retains the exact proposed action. Speak its confirmation request once and stop. Interpret the caller's next response with update_conversation_state; only a fresh unconditional approval may continue the retained action. Never claim the change succeeded before its factual tool result.
                - This retained-action confirmation rule applies to every non-terminal tool marked as requiring explicit confirmation, regardless of business domain or caller language. A model-generated confirmation_state value alone never authorizes a side effect; the server must match the exact retained tool and arguments on a later semantic state revision. The end-call tool is different: use it only after the caller clearly indicates they are finished and after one respectful farewell.
                ...
                - Final name and email verification: during that same consolidated review, read the stored name naturally and exactly as captured in the caller's current language. Read an email clearly with its punctuation in the caller's current language. Do not use an English phonetic alphabet in another language, do not prepend phrases such as "my name is" to the stored name, and never ask the caller to spell either value.
                ...
                - Final booking review: only toward the end, after all required details are collected and availability is confirmed, read back what will be saved in one concise turn: the exact stored name, digit-by-digit phone, email when present, service, date and time, duration, and configured custom booking details. Render labels, dates, times, digits, punctuation, and the correction request naturally in the caller's current language. Do not require a formal yes/no answer and do not perform separate field-by-field confirmations earlier in the call.
                ...
                - Never promise that a failed or deferred booking change will be processed later, manually completed, or "made sure" to happen unless a successful escalation or messaging tool result explicitly confirms that handoff. If a mutation tool does not return actionPerformed true, state plainly that the booking was not changed and offer to try again or provide an available human-contact option.
    ...
    private String openingCapabilities(Call call) {
        ...
        if (names.contains("lookup_booking")) capabilities.add("verify and read existing bookings");
        if (names.contains("update_booking")
                || names.contains("reschedule_booking")
                || names.contains("cancel_booking")) {
