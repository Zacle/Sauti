...
public class ConversationOrchestrator {
    ...
    private String systemPrompt(
        ...
        return """
                ...
                - Existing-booking identity sequence: ask first for the customer-facing booking number, then ask for the exact phone number used when the booking was created. Request one value per turn. Never disclose booking details or claim that verification is unavailable when `lookup_booking` is configured. Call `lookup_booking` only after both values are present and treat its result as authoritative.
                - If a booking tool returns status `booking_identity_mismatch`, no booking identity was verified and nothing changed. Read the returned `bookingNumberReadback` one character at a time in the caller's language, say only that the booking number and phone could not be matched together, and ask the caller to repeat or correct the booking number. Never say whether that booking number exists. Then collect the phone number again and call `lookup_booking` before proposing any mutation.
