...
public class ManagedVoiceToolService {
    ...
    private static final Set<String> PROVIDERS = Set.of("telnyx");
    private static final int MAX_AUTHORIZED_CHAIN_DEPTH = 4;
    ...
    /**
     * Executes a Telnyx server-side webhook tool after the webhook controller
     * has authenticated the shared secret and resolved the Sauti call id.
     */
    public Map<String, Object> executeTelnyxWebhook(
            String callSid,
            String invocationId,
            String name,
            JsonNode payload
        ...
        var call = callRepository.findByTwilioCallSid(callSid)
                ...
                .orElseThrow(() -> new IllegalArgumentException("Telnyx voice call is unavailable"));
        Map<String, Object> converted = payload == null || !payload.isObject()
                ? Map.of()
                : objectMapper.convertValue(payload, new TypeReference<>() { });
        var arguments = withoutNulls(converted);
        var normalizedInvocationId = "telnyx:webhook:"
                + firstNonBlank(invocationId, fingerprint(name, arguments));
        return executeCached("telnyx", call, normalizedInvocationId, name, arguments);
    }
    ...
    /**
     * Managed providers are inconsistent about following a returned
     * nextTool/nextToolArguments instruction. Those fields are generated by
    ...
    private LlmToolResult routeAuthorizedChain(Call call, LlmToolCall initial) {
        ...
        for (var depth = 0; depth < MAX_AUTHORIZED_CHAIN_DEPTH; depth++) {
            var routed = toolRouter.route(call, current);
            if (!routed.success()) return routed;
            var facts = routed.result();
            var nextName = value(facts, "nextTool");
            var rawArguments = facts.get("nextToolArguments");
            if (!Boolean.TRUE.equals(facts.get("nextToolAuthorized"))
                    || "end_call".equals(nextName)
                    || !nextName.matches("[A-Za-z][A-Za-z0-9_]{1,63}")
                    || !(rawArguments instanceof Map<?, ?> arguments)) {
                return routed;
            }
            var nextArguments = stringKeyed(arguments);
            var transition = nextName + ":" + fingerprint(nextName, nextArguments);
            if (!seen.add(transition)) {
                LOGGER.warn(
                        "Stopped repeated managed tool chain sautiCallId={} tool={}",
                        call.getId(), nextName
                );
                return routed;
            }
            LOGGER.info(
                    "Following authorized managed tool chain sautiCallId={} fromTool={} toTool={} depth={}",
                    call.getId(), current.name(), nextName, depth + 1
            );
            current = new LlmToolCall(
                    initial.id() + ":chain:" + (depth + 1),
                    nextName,
                    nextArguments
            );
        }
        LOGGER.warn(
                "Stopped managed tool chain at depth limit sautiCallId={} initialTool={}",
                call.getId(), initial.name()
        );
        return toolRouter.route(call, current);
    }

    private Map<String, Object> stringKeyed(Map<?, ?> values) {
