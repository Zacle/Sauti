            ...
            send(Map.of(
                    ...
                    "response", Map.of(
                            "instructions", "Answer exactly once in the current caller language using only configured facts. "
                                    ...
                                    + "Ask callers to provide identity details naturally; never ask them to use NATO phonetics or dictate a phone digit by digit. "
                                    + "Do not read fields back as they are collected. After every required detail is present and availability is confirmed, "
                                    + "the agent performs one consolidated final review, and the caller only confirms or corrects it."
                    )
            ));
        }
            ...
            send(Map.of(
                    ...
                    "response", Map.of(
                            "instructions", "Give one concise answer based only on the tool output and in the current "
                                    ...
                                    + "For identity_confirmation_required, all intake is complete: the agent must speak one consolidated final review using "
                                    + "agentReadback and bookingReview, then ask whether everything is correct. Never split it into field-by-field confirmations "
                                    + "or ask the caller to produce NATO phonetics or digit-by-digit dictation.",
                            "tool_choice", "none"
                    )
            ));
        }
