...
final class TwoCheckoutTenantReference {
    private static final String PREFIX = "sauti_";
    private final byte[] secret;

    TwoCheckoutTenantReference(String secret) {
    ...
    UUID verify(String reference) {
        ...
        if (!value.startsWith(PREFIX)) throw new SecurityException("Invalid 2Checkout customer reference");
        var parts = value.substring(PREFIX.length()).split("_", -1);
        if (parts.length != 2 || parts[0].length() != 32) {
            throw new SecurityException("Invalid 2Checkout customer reference");
        }
    ...
    private String signature(String value) {
        try {
            ...
            throw new IllegalStateException("Could not protect 2Checkout customer reference", exception);
        }
    }

    private void requireConfigured() {
        if (secret.length == 0) throw new IllegalStateException("2Checkout is not configured");
    }
