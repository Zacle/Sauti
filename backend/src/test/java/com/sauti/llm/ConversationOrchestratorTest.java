...
class ConversationOrchestratorTest {
    ...
    void loopsThroughToolCallAndReturnsFollowUpResponse() {
        ...
        assertThat(provider.contexts.get(0).systemPrompt())
                ...
                .contains("Existing-booking identity sequence for every agent, language, and business type")
                .contains("phone number used for the booking")
                .contains("exact existing appointment time")
                .contains("never an identity factor")
                .contains("booking_not_found")
                .contains("Never reveal a stored name, candidate times")
                .contains("Do not restart phone/date/time")
                .contains("final four letters or digits")
                .contains("Never use the phrase \"nothing was changed\"")
                .contains("server privately retains the tenant-bound booking identity")
                ...
                .contains("read the stored name naturally and exactly as captured")
                .contains("Do not use an English phonetic alphabet in another language")
                ...
                .contains("Never infer it from the recipient's gender")
                .contains("phonetic similarity alone")
                .contains("do not record a service yet")
                .contains("private review token")
                .contains("Interpret every response to that server review with `update_conversation_state` first")
                .contains("fresh unconditional confirmation")
                .contains("`question_handling`", "`answer_before_action`")
                .contains("Never say \"oops\"")
                .contains("Never initiate a goodbye merely because you finished answering one question")
    ...
    void realtimeInstructionsKeepTheConfiguredBusinessRoleAndHours() {
        ...
        assertThat(instructions)
                ...
                .contains("Never answer a direct customer question only with another question")
                .contains("state the exact configured open days and opening and closing times")
                .contains("Do not ask for a day or a morning/afternoon preference instead of answering")
    ...
    void managedRealtimeInstructionsRetainQualityAndSafetyWithASmallerStablePrefix() {
        ...
        assertThat(managed)
                ...
                .contains("Opening language hint: en")
                .contains("Supported caller languages: en, fr")
                .contains("Automatically follow a supported language")
                .contains("Do not ask the caller to choose a language")
                .contains("one question for one missing value")
                .contains("persist all of them together")
                .contains("Never ask again for a value already supplied")
                .contains("Ask for only one")
                .contains("accept every clear value the caller supplies together")
                .contains("check_availability")
                .contains("book_slot")
                .contains("update_conversation_state")
                .contains("Preserve leading zeroes and repeated digits")
                .contains("Short clear digit groups are retained privately")
                .contains("partialPhoneDigits instruction")
                .contains("Read a completed number digit by digit only in the final review")
