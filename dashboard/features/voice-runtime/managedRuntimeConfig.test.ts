...
test("managed provider errors do not stringify arbitrary provider objects", () => {
  assert.equal(providerError("Telnyx", { apiKey: "secret" }), "Telnyx ended the voice session unexpectedly.");
  assert.equal(providerError("Telnyx", new Error("connection refused")), "Telnyx: connection refused");
  assert.equal(
    providerError("Telnyx", "token=secret wss://provider.example/session?token=secret"),
    "Telnyx: token [redacted] [provider endpoint]",
  );
  assert.equal(
    providerError("Telnyx", "Server error: Unknown error", {
    ...
    "Telnyx: Server error: Unknown error (type=tool_execution, code=500, debug=token [redacted] failed)",
  );
});
